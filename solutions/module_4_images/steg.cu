...
__global__
void encrypt(unsigned char* pixel_array, int width, int height, char* msg){
    ...
    if(x < width && y < height){
        ...
        // Encode this character of the msg in the R val of this pixel
        if (offset < MSG_LEN) {
            //printf("Offset less than 177\n");
            //printf("Offset is: %d\n", offset);
            //printf("msg[%d]: %c\n", offset, msg[offset]);
            //printf("param: %d\n", param);
...
int main(int argc, char** argv){
    ...
    char msg[] = "";
    ...
    cudaMalloc(&gpu_msg,strlen(msg)+1);
    ...
    cudaMemcpy(gpu_msg,msg,strlen(msg)+1, cudaMemcpyHostToDevice);
    ...
    encrypt<<<dimGrid,dimBlock>>>(gpu_pixels_array,myImage->width,myImage->height,gpu_msg);
